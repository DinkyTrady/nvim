...
map('n', ']q', function()
  ...
  local ok_cnext = pcall(vim.api.nvim_command, 'cnext')
...
map('n', '[q', function()
  ...
  local ok = pcall(vim.api.nvim_command, 'cprev')
  if not ok then
    vim.cmd 'clast'
  end
end, 'Prev Quickfix')
...
map('n', ']l', function()
  ...
  local ok = pcall(vim.api.nvim_command, 'lnext')
  if not ok then
    vim.cmd 'lfirst'
  end
...
map('n', '[l', function()
  ...
  local ok = pcall(vim.api.nvim_command, 'lprev')
  if not ok then
    vim.cmd 'llast'
  end
